...
{   
    ...
    {
        // handle the case when no substitutions are required
        bool no_subs = std::find((fmt_ptr) format_, end(), '%') == end();
        if(no_subs)
            return format_;
        // inplace_format needs more arguments than provided
        assert(format_start_ == end()); 
        return buf_;
    }
...
template<size_t buf_size, size_t format_size>
std::string str(inplace_format_t<buf_size, format_size> const& fmt)
{
    return fmt.str();
}
