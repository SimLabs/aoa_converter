...
struct aoa_writer
{
    static constexpr uint32_t c_GP_Version = 300;
    static constexpr uint32_t c_GP_BaseFileMarker = ('D' << 8) | 'O';
    static constexpr uint32_t AOD_HEADER_SIZE = sizeof(c_GP_Version) + sizeof(c_GP_BaseFileMarker) + 4 * sizeof(uint32_t);
